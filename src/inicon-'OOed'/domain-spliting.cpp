...
int main(int argc,char **argv){
  const int D=2,Ntri=6,split_type=1;
  int err,l; 
  ...
  if(split_type==0){
    ...
    mdel = new Domain (D,1<<D,0,xi,xf); 
  }
  else if(split_type==1){
    err=unit2_hexagon(Ntri,D,xv);if(err!=0) return err;
    mdel = new Domain (D,Ntri,1,xv); 
  }
  
  cout << "split\n";
  err=mdel->domain_split(cutoff,F); if(err!=0) return err;
  
  cout << "clean\n";
  err=mdel->clean_domain();if(err!=0) return err;
  
  cout << "print\n";  
  err=mdel->print_sph("SPH-particles.dat"); if(err!=0) return err;
