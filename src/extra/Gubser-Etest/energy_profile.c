...
double gubser_transverse_entropy(double r,void *p){
  double Q=*(double*)p,foo,lambda;
  lambda = 1.+2.*Q*Q*(1+r*r)+Q*Q*Q*Q*(1-r*r)*(1-r*r);
  foo = (r*(1+Q*Q*(1+r*r)*(1+r*r) ) )/sqrt(lambda*lambda*lambda);
  return foo;
}
...
int main(){
  ...
  for(tau=tau0;tau<tauM;tau+=dt){
    ...
    F.function = &gubser_transverse_entropy;
    err=gsl_integration_qagiu(&F,0.0,eabs,erel,limit,w,&S,&abserr);

    fprintf(dadosout,"%.8f %.8f %.8f %.8f %d %lf %lf\n",tau,I,S,abserr,w->size,log(tau),log(I));
  }
